...
- **2026-09-20（Phase 15/16 验收关闭）**：用户确认 CC 与 Codex 连接器真机全部正常（「现在没问题了」）。验收过程共追加五批修复（v0.6.1 dev 代理 origin 重写 + disable 接线 / v0.6.2 Rust regex matcher 改路由分类 / v0.6.3 CC Notification 污染摘除 / v0.6.4 node sink 传输重做 + 重装堆积修复），全部根因坐实并记录于 docs/07/08。**连接器三件套（zcode/CC/Codex）至此全部真机验收通过。**

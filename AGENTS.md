...
- **2026-09-21（晚）：Phase 18 设置窗重构代码完成（插件分页 + 每页 取消/应用，详见 docs/05 Phase 18）**：插件分类挪导航底部可展开、每插件独立子页（设置内容**始终展示**可先配后启用，错误边界隔离单卡崩溃）；保存模型统一 Windows 式 取消/应用——**每页独立草稿+底栏**（原四条保存通路不一致、roamer/stats-hud 卡 fire-and-forget 静默丢失，一并收口），脏页切导航弹守卫条（留下/丢弃并离开/保存并离开）；卡契约受控化（`PluginSettingsCardProps {value, onChange}`，卡内零网络）+ `configStore` 自有存储缝（physics 页走其自有路由，overlay 传播链路不变）；宠物编辑器 iframe 保留自带草稿（例外）。上游 physics `760fb37`：PhysicsCard 可选受控模式（DSH 侧逐字节不变，上游 194 全绿）+ desktop 入口 configStore。桌面 420 用例全绿（+7：draft 纯逻辑/受控卡契约）；**真机验收待用户（docs/05 Phase 18 验收清单）**。

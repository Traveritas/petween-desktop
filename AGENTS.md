...
├── vendor/petween          ← git submodule → github.com/Traveritas/petween（锁定 84cebfe = 11830e5 + 粒子特效扩容）
└── vendor/petween-physics  ← git submodule → github.com/Traveritas/petween-physics（锁定 02b582a）
```
...
2. **全程同源**：petween client 侧 HTTP 全是根相对路径，prod 各窗口 `loadURL('http://127.0.0.1:<random>/...')`，dev 用 electron-vite `server.proxy` 代理 `/api/petween`、`/api/petween-desktop`、`/api/petween-physics`、`/petween-assets` 到 local-server（编辑器 iframe 直接指向 local-server origin，不走代理）——这是「零改动 petween」的前提。
...
8. **不 import `vendor/petween/src/client/index.ts`**（那是 DSH slot 注册）；overlay 入口自己 mount `PetOverlay`（纯组件）。设置页是壳层自建页（Phase 18：每页草稿+取消/应用），宠物分区以 iframe 内嵌 host 伺服的 `/petween-editor/`（`lib/editor.js` 自包含 IIFE，保留编辑器自带草稿模型）。
...
12. **设置窗保存模型与插件卡契约（Phase 18）**：每页独立草稿 + 底部 取消/应用（`src/renderer/settings/page-draft.tsx` 的 `usePageDraft`），脏页切导航走守卫条（留下/丢弃并离开/保存并离开）；宠物编辑器 iframe 例外（自带草稿）。companion 设置卡是**受控组件**（`PluginSettingsCardProps {value, onChange}`，卡内零网络，资产上传为唯一明示例外）；自有存储的插件声明 `configStore`（load/save，physics 走其自有路由，save 返回归一化结果供 baseline 采纳）。写设置一律切片 PUT（服务端深合并 per-id），**改动单页保存语义前先看 page-draft 的 in-flight 保护与 per-id 钉测**。
...
- **2026-09-19：Phase 13/14 手感二三批代码完成**（上游 `382f148` 多选/框选/undo/右键菜单 + `be701c5` bezier 曲线编辑器，已随 `50942a5` 前的指针落库、含在当前基线内；详见 docs/05 Phase 11 对应节；**真机验收随编辑器线顺延**）。
...
- petween 基线：64 测试文件 / 1115 用例全绿（`84cebfe` = `11830e5`（Phase 18 保存文案对齐）+ 粒子特效扩容六枚举；`6ed667e` 起 `changePoseWithinActive` 默认 true——2026-09-18 用户拍板，桌面/DSH 两宿主共用）；petween-physics 基线：12 文件 / 195 用例（`6e1ef02` = `760fb37` 受控模式 + same-site 栅栏落穿 + configStore 诊断/归一化返回）。
...
- **2026-09-21：Phase 17 roamer companion 代码完成**（宠物自主行为：游荡/待机动作/捣乱，详见 docs/05 Phase 17）：零 petween core 改动的第三个 in-tree companion（`src/renderer/companions/roamer/`：纯决策引擎 + 薄 runtime + windows DOM 层 + host 动画装配）；PositionDriver 租约仅走 leg 期间持有（与 physics 抛掷互让）；拉窗/便签/内容池 + 四待机动作 + 冲过/探头（朝向镜像因 pose zoom 校验域无负值砍掉）；三类行为子项全独立开关（`companions.options['roamer']`，3s 轮询热生效）；in-tree 不建仓（用户问询确认，physics 迁移模板已验证）。413 用例全绿（+38）；**✅ 2026-09-21 真机验收通过（用户结论「基本没问题」，docs/05 Phase 17 验收清单）**。
- **2026-09-21（晚）：Phase 18 设置窗重构代码完成（插件分页 + 每页 取消/应用，详见 docs/05 Phase 18）**：插件分类挪导航底部可展开、每插件独立子页（设置内容**始终展示**可先配后启用，错误边界隔离单卡崩溃）；保存模型统一 Windows 式 取消/应用——**每页独立草稿+底栏**（原四条保存通路不一致、roamer/stats-hud 卡 fire-and-forget 静默丢失，一并收口），脏页切导航弹守卫条（留下/丢弃并离开/保存并离开）；卡契约受控化（`PluginSettingsCardProps {value, onChange}`，卡内零网络）+ `configStore` 自有存储缝（physics 页走其自有路由，overlay 传播链路不变）；宠物编辑器 iframe 保留自带草稿（例外）。上游 physics `760fb37`：PhysicsCard 可选受控模式（DSH 侧逐字节不变，上游 194 全绿）+ desktop 入口 configStore。桌面 420 用例全绿（+7：draft 纯逻辑/受控卡契约）；**✅ 2026-09-21 真机验收通过**。
- **2026-09-21（夜）：宠物编辑器保存文案对齐（上游 petween `11830e5`）**：核查确认编辑器 store 本就是显式保存模型（saveConfig=应用 / revertConfig=取消），用户拍板**纯文案批**（行为零变化）：SaveIndicator 按钮固定 应用/取消、目标宠物名挪状态行；切换门控/三条 notice/卡片提示/编辑器副标题同词表；「取消」保留确认弹窗。上游 1111 全绿（远端 README 提交 `cb016d8` rebase 合入）；桌面零代码改动，重建 editor.js + bump 指针即生效。遗留：脏时切换宠物仍是禁止+出口提示（三选原语齐备，需要再做）。
- **2026-09-21（深夜）：粒子特效扩容（上游双 push，桌面零代码）**：petween `84cebfe` 三新特效（heart-burst 爱心负重力上浮 / petal-fall 落樱缓坠 / firework 大半径混合爆发）+ 三新形状（heart clip-path / ring 空心描边圆——唯一非填充形状 / petal 椭圆），`ParticleEffectId`/`PARTICLE_EFFECT_IDS`/编辑器下拉三处同步，新增两条防漂移测试（渲染表↔枚举互查、验证器全枚举通过），motion-format.md 规格表更新，1111→1115 全绿。连带修复：petween-physics 镜像类型硬编码三特效 union，真实服务扩到六字面量后方法双变两条赋值路均断（桌面 typecheck 红）——按镜像兼容规则上游扩六字面量（`02b582a`，194 全绿，physics 发送侧零变化）。桌面零改动即得六特效，420 全绿。**音效插件化评估结论**（同轮沉淀）：可行——`subscribeAnimation`（start 带 definitionId）+ host 动画库取定义自排定时，可覆盖转场/交互动效的逐事件时机与 pose/pointer/state 触发面；缺口仅循环动画事件逐圈重放（start 只发一次）与无逐事件流，V1 音效不必动上游。详见 docs/05 同日追记。
- **2026-09-21（深夜）：v0.8.0 里程碑——六路综合评审**（范围 = Phase 17 四批 + Phase 18 + 两 bump + 对应 submodule；主进程/渲染-设置窗/渲染-roamer/测试/文档/安全，roamer 首评故渲染拆两路）：**零 P0、6 P1 全修**（apply in-flight 编辑保护/守卫 dirty 复位/端口草稿回滚/addImage 过期闭包/保存模型抽取可测+5 用例/per-id 合并钉测）+ 12 项 P2 当场修（守卫条收起、physics configStore 归一化返回+400 诊断（上游 `6e1ef02`：另含 same-site 栅栏落穿 Origin 比对）、roamer 拉窗视口比例预算/stage 时钟注入/mischief 关闭复查/URL 白名单/closeAll 死码、受控卡契约口径收窄）+ 文档批（docs/02 拓扑与基线、docs/05 验收状态与数字与划线、AGENTS 决策 12 新增/决策 2·8 刷新/Phase 13·14 补行、README 两特性）。**427 用例全绿（+7）后打标签 v0.8.0**。Backlog：roamer runtime 行为测试（评审给出四例设计）、engine 补测四项、tests 纳入 typecheck、守卫覆盖 iframe 脏状态（postMessage 桥）——详见 docs/05「v0.8.0 里程碑评审」。Phase 17（roamer）真机验收 ✅（2026-09-21 用户确认「基本没问题」）。
- **2026-09-21（roamer 真机反馈批）**：用户三项反馈全修——①**窗口/便签停留可配**（`mischief.pull/noteLingerMs` 0.5~60s，卡片秒单位 + >10s 遮挡警告行；便签默认 90s→45s 收进上限）；②**样式重做**（拉窗标题栏三圆点+题注入栏；便签胶带+横线纸/图片拍立得；便签图片裁剪根因 = 190px 皮与 img 300px 上限打架，img 改随容器 100%；**便签旋转真因 = pop 关键帧 fill-mode both 永久钉死 transform 杀死内联 rotate**，改 `--pt-rot` 变量进关键帧）；③**冲过/探头没看见变化**的双重根因 = 两动作默认关 + 表现与走路无异——dash 加 `gait:'dash'` 标记播专用冲刺步态（前倾+快摆+拉伸 loop `user:roamer-dash`），peek 倾身 44→76px + 旋转、2.6→3.2s。428 用例全绿（+1）。
